...
TOOLS = [
    ...
    {"id": "word_to_pdf","title": "Word to PDF","desc": "Convert DOCX files into PDF for sharing and printing.","icon": "📝","status": "ready"},
]
...
#______________convert word to pdf_____

def render_word_to_pdf() -> None:
    topbar()
    st.markdown("## 📝 Word → PDF")
    st.caption("Convert DOCX to PDF. Best for resumes, reports, assignments, and office documents.")

    uploaded = st.file_uploader("Upload Word file(s)", type=["docx"], accept_multiple_files=True)

    col1, col2 = st.columns([2, 1])
    with col1:
        out_name_mode = st.selectbox("Output name", ["Keep original name", "Custom name"], index=0)
    with col2:
        custom_name = st.text_input("Custom PDF name", value="converted.pdf", disabled=(out_name_mode != "Custom name"))
    ...
    if run and uploaded:
        for f in uploaded:
            try:
                ...
                pdf_bytes = convert_docx_to_pdf_bytes(docx_bytes, input_filename=f.name)

                if out_name_mode == "Custom name":
                    filename = custom_name if custom_name.lower().endswith(".pdf") else custom_name + ".pdf"
                else:
                    filename = Path(f.name).stem + ".pdf"
